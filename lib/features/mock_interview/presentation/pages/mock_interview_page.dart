  ...
  Future<void> _initializeInterview() async {
    ...
    try {
      ...
      final greeting = await firebaseAI.generateText(
        prompt: '''
You are an AI interview coach conducting mock interviews. 

Start with a warm, friendly greeting and then ask the user: "In which field would you like to practice your interview? (e.g., Software Development, Data Science, Product Management, Marketing, Finance, etc.)"

Keep it brief, professional, and welcoming. Don't ask any interview questions yet - just greet and ask for their field preference.
''',
      );
      ...
      setState(() {
        _messages.add(ChatMessage(
          text: e.toString().contains('Firebase') || e.toString().contains('not configured')
              ? '⚠️ Firebase AI is not configured. Please ensure Firebase is properly set up to use AI interview features.\n\nWelcome! I\'m your AI interview coach. In which field would you like to practice your interview? (e.g., Software Development, Data Science, Product Management, etc.)'
              : 'Welcome! I\'m your AI interview coach. In which field would you like to practice your interview? (e.g., Software Development, Data Science, Product Management, etc.)',
          isUser: false,
        ));
        _isLoading = false;
  ...
  Future<void> _sendMessage() async {
    ...
    try {
      ...
      if (_interviewState == InterviewState.greeting) {
        // User provided their field preference
        _selectedField = text;
        setState(() {
        ...
        await _generateNextQuestion(firebaseAI, null);
      }
    } catch (e) {
      setState(() {
        _messages.add(ChatMessage(
...
''';
      ...
      final prompt = '''
Generate a mock interview question for someone practicing interviews in the field of: $field

Create a relevant, professional interview question with exactly 4 multiple choice options. This should be a typical question asked in interviews for this field.
...
''';
      ...
      final fallbackResponse = await firebaseAI.generateText(
        prompt: '''
You are conducting a mock interview. The user wants to practice in the field: $field

Acknowledge their field choice briefly (1 sentence) and ask the first interview question relevant to $field. Make it a typical question for this field.
''',
      );
...
''';
      ...
      if (selectedAnswerIndex != null && _currentQuestion != null) {
        ...
        prompt = '''
The user is practicing mock interviews in the field: ${_selectedField ?? 'their chosen field'}

Previous question: ${_currentQuestion!.question}
User selected: $selectedOption (${wasCorrect ? 'Correct' : 'Incorrect'})

Generate the next interview question for this field. Create a relevant, professional interview question with exactly 4 multiple choice options.
...
''';
        ...
        prompt = '''
Generate a mock interview question for someone practicing interviews in the field: ${_selectedField ?? 'their chosen field'}
...
''';
      ...
      final fallbackResponse = await firebaseAI.generateText(
        prompt: '''
You are conducting a mock interview. The user is practicing in the field: ${_selectedField ?? 'their chosen field'}

Ask the next interview question relevant to this field. Make it a typical question for interviews in this field.
''',
      );
                          ...
                          child: TextField(
                            ...
                            decoration: InputDecoration(
                              hintText: _interviewState == InterviewState.greeting
                                  ? 'Enter your field (e.g., Software Development, Data Science...)'
                                  : 'Type your answer...',
