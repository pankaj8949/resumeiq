...
/// Service for generating HTML templates from ResumeEntity
...
''';
  ...
  static String _generateHeader(PersonalInfoEntity? personalInfo) {
    ...
    if (personalInfo.linkedInUrl != null && personalInfo.linkedInUrl!.isNotEmpty) {
      contactItems.add('💼 <a href="${_escapeHtml(personalInfo.linkedInUrl!)}">LinkedIn</a>');
    }
    if (personalInfo.githubUrl != null && personalInfo.githubUrl!.isNotEmpty) {
      contactItems.add('💻 <a href="${_escapeHtml(personalInfo.githubUrl!)}">GitHub</a>');
    }
    if (personalInfo.portfolioUrl != null && personalInfo.portfolioUrl!.isNotEmpty) {
      contactItems.add('🌐 <a href="${_escapeHtml(personalInfo.portfolioUrl!)}">Portfolio</a>');
    }
...
''';
  ...
  static String _generateExperience(List<ExperienceEntity> experience) {
    if (experience.isEmpty) return '';
    
    final items = experience.map((exp) {
      final position = _escapeHtml(exp.position);
      final company = _escapeHtml(exp.company);
      final location = exp.location != null && exp.location!.isNotEmpty ? _escapeHtml(exp.location!) : '';
      final dateRange = _formatDateRange(exp.startDate, exp.endDate, exp.isCurrentRole ?? false);
      final responsibilities = exp.responsibilities.map((r) => '• ${_escapeHtml(r)}').join('<br>');
...
''';
  ...
  static String _generateEducation(List<EducationEntity> education) {
    if (education.isEmpty) return '';
    
    final items = education.map((edu) {
      final degree = _escapeHtml(edu.degree);
      final institution = _escapeHtml(edu.institution);
      final fieldOfStudy = edu.fieldOfStudy != null && edu.fieldOfStudy!.isNotEmpty ? ', ${_escapeHtml(edu.fieldOfStudy!)}' : '';
      final dateRange = _formatDateRange(edu.startDate, edu.endDate, false);
      final gpa = edu.gpa != null ? ' | GPA: ${edu.gpa!.toStringAsFixed(2)}' : '';
...
''';
  ...
  static String _generateProjects(List<ProjectEntity> projects) {
    if (projects.isEmpty) return '';
    
    final items = projects.map((proj) {
      final name = _escapeHtml(proj.name);
      final description = _escapeHtml(proj.description ?? '');
      final technologies = proj.technologies != null && proj.technologies!.isNotEmpty ? 'Technologies: ${_escapeHtml(proj.technologies!)}' : '';
      final url = proj.url != null && proj.url!.isNotEmpty ? '<br><a href="${_escapeHtml(proj.url!)}">View Project</a>' : '';
...
''';
  ...
  static String _generateCertifications(List<CertificationEntity> certifications) {
    if (certifications.isEmpty) return '';
    
    final items = certifications.map((cert) {
      final name = _escapeHtml(cert.name);
      final issuer = cert.issuer != null && cert.issuer!.isNotEmpty ? ' - ${_escapeHtml(cert.issuer!)}' : '';
      final date = cert.issueDate != null ? ' (${_formatDate(cert.issueDate)})' : '';
      final expiryDate = cert.expiryDate != null ? ' - Expires: ${_formatDate(cert.expiryDate)}' : '';
...
''';
  ...
  static String _generateModernTemplate(ResumeEntity resume) {
    final content = '''
...
''';
  ...
  static String _generateMinimalTemplate(ResumeEntity resume) {
    final content = '''
${_generateHeader(resume.personalInfo)}
${_generateSummary(resume.summary)}
${_generateExperience(resume.experience)}
...
''',
  ...
  static String _generateExecutiveTemplate(ResumeEntity resume) {
    return _generateModernTemplate(resume); // Same structure, different styling handled via CSS
  }

  /// Developer Template
  static String _generateDeveloperTemplate(ResumeEntity resume) {
    return _generateModernTemplate(resume);
  }

  /// Creative Template
  static String _generateCreativeTemplate(ResumeEntity resume) {
    return _generateModernTemplate(resume);
  }

  /// Graduate Template
  static String _generateGraduateTemplate(ResumeEntity resume) {
    final content = '''
${_generateHeader(resume.personalInfo)}
${_generateSummary(resume.summary)}
${_generateEducation(resume.education)}
...
''';
  ...
  /// Academic Template
  static String _generateAcademicTemplate(ResumeEntity resume) {
    return _generateGraduateTemplate(resume);
  }

  /// Product Manager Template
  static String _generateProductManagerTemplate(ResumeEntity resume) {
    return _generateModernTemplate(resume);
  }

  /// Data Analyst Template
  static String _generateDataAnalystTemplate(ResumeEntity resume) {
    return _generateModernTemplate(resume);
  }

  /// Marketing Template
  static String _generateMarketingTemplate(ResumeEntity resume) {
    return _generateModernTemplate(resume);
  }

  /// Sales Template
  static String _generateSalesTemplate(ResumeEntity resume) {
    return _generateModernTemplate(resume);
  }

  /// Finance Template
  static String _generateFinanceTemplate(ResumeEntity resume) {
    return _generateExecutiveTemplate(resume);
  }

  /// Operations Template
  static String _generateOperationsTemplate(ResumeEntity resume) {
    return _generateModernTemplate(resume);
  }

  /// Two Column Template
  static String _generateTwoColumnTemplate(ResumeEntity resume) {
    final sidebar = '''
...
''';
  ...
  static String _generateCompactTemplate(ResumeEntity resume) {
    final content = '''
${_generateHeader(resume.personalInfo)}
${_generateSummary(resume.summary)}
...
''',
  ...
  /// Classic Template
  static String _generateClassicTemplate(ResumeEntity resume) {
    return _generateModernTemplate(resume);
  }

  /// Tech Lead Template
  static String _generateTechLeadTemplate(ResumeEntity resume) {
    return _generateModernTemplate(resume);
  }

  /// Consultant Template
  static String _generateConsultantTemplate(ResumeEntity resume) {
    return _generateExecutiveTemplate(resume);
  }

  /// Executive Summary Template
  static String _generateExecutiveSummaryTemplate(ResumeEntity resume) {
...
''';
  ...
  static String _generateAtsOptimizedTemplate(ResumeEntity resume) {
    final content = '''
${_generateHeader(resume.personalInfo)}
${_generateSummary(resume.summary)}
${_generateExperience(resume.experience)}
