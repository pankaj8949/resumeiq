...
class ResumeBuilderPage extends ConsumerStatefulWidget {
  const ResumeBuilderPage({super.key, this.resumeId});
...
class _ResumeBuilderPageState extends ConsumerState<ResumeBuilderPage> {
  ...
  Future<void> _loadResume(String resumeId) async {
    ...
    try {
      resume = resumeState.resumes.firstWhere(
        (r) => r.id == resumeId,
      );
    } catch (e) {
    ...
    if (resume == null) {
      resume = await ref.read(resumeNotifierProvider.notifier).loadResume(resumeId);
    }
  ...
  Future<void> _handleGenerateSummary() async {
    ...
    final summary = await ref.read(resumeNotifierProvider.notifier).generateSummary(
  ...
  Future<void> _handleSave() async {
    ...
    final bool success;
    if (widget.resumeId != null && widget.resumeId!.isNotEmpty) {
      success = await ref.read(resumeNotifierProvider.notifier).updateResume(resume);
    } else {
      success = await ref.read(resumeNotifierProvider.notifier).createResume(resume);
    }
    ...
    if (success) {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(widget.resumeId != null 
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(ref.read(resumeNotifierProvider).error ?? 'Failed to save resume'),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  Widget build(BuildContext context) {
    ...
    if (_isLoadingResume || (widget.resumeId != null && resumeState.isLoading && resumeState.currentResume == null)) {
      return Scaffold(
        appBar: AppBar(
      ...
      child: Column(
        ...
        children: [
          CustomTextField(
            ...
            validator: (value) => Validators.required(value, fieldName: 'Resume title'),
          ),
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
            ...
            child: Column(
              ...
              children: [
                ...
                ListTile(
                  ...
                  subtitle: Text(startDate != null ? AppDateUtils.DateUtils.formatDate(startDate, format: 'MMM yyyy') : 'Select date'),
                ...
                ListTile(
                  ...
                  subtitle: Text(endDate != null ? AppDateUtils.DateUtils.formatDate(endDate, format: 'MMM yyyy') : 'Select date (optional)'),
                  trailing: const Icon(Icons.calendar_today),
                  onTap: () async {
                    final date = await showDatePicker(
                      context: context,
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                if (institutionController.text.trim().isEmpty || degreeController.text.trim().isEmpty) {
                  ScaffoldMessenger.of(context).showSnackBar(
                ...
                setState(() {
                  _educationList.add(EducationEntity(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
            ...
            child: Column(
              ...
              children: [
                ...
                CheckboxListTile(
                  ...
                  value: isCurrent,
                  onChanged: (value) => setDialogState(() => isCurrent = value ?? false),
                ),
                if (!isCurrent) ...[
                  ListTile(
                    title: const Text('Start Date'),
                  ...
                  ListTile(
                    title: const Text('End Date'),
                    subtitle: Text(endDate != null ? AppDateUtils.DateUtils.formatDate(endDate, format: 'MMM yyyy') : 'Select date'),
                    trailing: const Icon(Icons.calendar_today),
                    onTap: () async {
                      final date = await showDatePicker(
                        context: context,
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                if (companyController.text.trim().isEmpty || positionController.text.trim().isEmpty) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('Please fill required fields')),
                  );
                  return;
                ...
                setState(() {
                  _experienceList.add(ExperienceEntity(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              setState(() {
                _projectsList.add(ProjectEntity(
            ...
            child: Column(
              ...
              children: [
                ...
                ListTile(
                  ...
                  subtitle: Text(issueDate != null ? AppDateUtils.DateUtils.formatDate(issueDate, format: 'MMM yyyy') : 'Select date'),
                  trailing: const Icon(Icons.calendar_today),
                  onTap: () async {
                    final date = await showDatePicker(
                      context: context,
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                if (nameController.text.trim().isEmpty) return;
                setState(() {
                  _certificationsList.add(CertificationEntity(
                    name: nameController.text.trim(),
